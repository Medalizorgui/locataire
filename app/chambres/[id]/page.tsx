...
const MonthCard = ({ 
  ...
  const total = month.montantEau + month.montantElectricite + month.fraisLouer
        ...
        <div className="bg-green-50 p-3 rounded">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="font-semibold text-green-800 border-t pt-1 mt-2">
              <span>Total:</span> {total.toFixed(2)} TND
            </div>
          </div>
        </div>
      </div>
...
// Main component
export default function ChambreDetailsPage({ params }: ChambreDetailsPageProps) {
  const { id } = use(params)
  ...
  // Computed values
  const stats = useMemo(() => {
    if (!chambre?.months) return { unpaidMonths: 0, totalMonths: 0, totalOwed: 0 }
    ...
    return { unpaidMonths, totalMonths, totalOwed }
  }, [chambre?.months])
